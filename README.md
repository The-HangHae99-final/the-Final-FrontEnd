...
[팀 소개 페이지](https://www.notion.so/1-SA-269179d67de145ab9a6a1135fdb5e297)
...
선정이유
💡 로그인 여부에 따라 헤더에서 보여져야 하는 UI를 다르게 만드려고 여러개의 props들을 헤더 내부의 컴포넌트들로 전해주는 과정에서 불편함을 느꼈습니다.로그인 여부, 유저정보 관련 데이터들을 툴킷으로 전역에서 관리하니 depth가 깊어질 수록 코드를 짜기가 복잡해지는 props 방식보다 간편합니다. 비동기관리나 불변성 유지를 하기 위해 따로 설치해야하는 redux-thunk나 immer도 이미 내재되어있는 이점도 있기 떄문에 선택했습니다.

**toolkit을 도입한 이유**

- redux는 깔아야할게 너무 많다(redux-thunk, immer 등)
- 스토어 설정이 복잡하다
- 요구하는 boiler plate가 너무 많다(action, action creator 등)

### Portal

선정이유
💡 처음엔 컴포넌트 안에서 Modal이라는 div를 따로 만들고, z-index를 높여서 현재 페이지의 화면을 덮도록 만들었습니다. 그런데 이번 프로젝트에서 새로운 페이지 컴포넌트를 띄워 정보를 보여주는 것보다 모달을 많이 쓰게 될 것 같아 사용성을 높이고 조금 더 쓰기 편한 방식으로 모달을 쓸 순 없을까 고민 했습니다. portal은 z-index 같은 부모-자식 간의 제약을 넘어서 dom노드에서 실제론 부모-자식관계이지만 자식이 부모로부터 독립적인 위치에 있을 수 있게 한다는 글을 보고 portal로 모달을 구현해보았습니다. z-index 효과를 줬을 때보다 화면을 구성 할 떄 메모리도 덜 차지하고 렌더링 시간도 빨라지게 됩니다.
